...
Matter.Events.on(physics, "beforeUpdate", () => {
  if (!race) return;
  const centerY = (TRACK.top + TRACK.bottom) / 2;
  for (const runner of race.runners) {
    if (runner.finished || !runner.body) continue;
    const position = runner.body.position, velocity = runner.body.velocity;
    const preferredY = centerY + Math.sin(position.x / 680 + runner.rank * 0.11) * 145;
    const forceY = Math.max(-0.0004, Math.min(0.0004, (preferredY - position.y) * 0.0000016 - velocity.y * 0.000025));
    Matter.Body.applyForce(runner.body, position, { x: 0, y: forceY });
  }
});
...
const FLAG_ISO_CODES = { ESP:"es", ARG:"ar", FRA:"fr", ENG:"gb-eng", BRA:"br", MAR:"ma", POR:"pt", BEL:"be", NED:"nl", MEX:"mx", COL:"co", GER:"de", CRO:"hr", SUI:"ch", ITA:"it", USA:"us", JPN:"jp", SEN:"sn", NOR:"no", URU:"uy", DEN:"dk", IRN:"ir", AUT:"at", EGY:"eg", ECU:"ec", NGA:"ng", TUR:"tr", AUS:"au", ALG:"dz", CAN:"ca", CIV:"ci", KOR:"kr", UKR:"ua", PAR:"py", RUS:"ru", POL:"pl", SWE:"se", WAL:"gb-wls", HUN:"hu", SRB:"rs", COD:"cd", SCO:"gb-sct", CMR:"cm", PAN:"pa", SVK:"sk", GRE:"gr", VEN:"ve", CZE:"cz", CHI:"cl", PER:"pe", CRC:"cr", ROU:"ro", MLI:"ml", RSA:"za", IRL:"ie", SVN:"si", TUN:"tn", KSA:"sa", QAT:"qa", UZB:"uz", BIH:"ba", BFA:"bf", IRQ:"iq", CPV:"cv", GHA:"gh", HON:"hn", ALB:"al", UAE:"ae", MKD:"mk", NIR:"gb-nir", JAM:"jm", GEO:"ge", JOR:"jo", ISL:"is", FIN:"fi", ISR:"il", BOL:"bo", KOS:"xk", OMA:"om", MNE:"me", GUI:"gn", CUW:"cw", SYR:"sy", GAB:"ga", BUL:"bg", NZL:"nz", ANG:"ao", HAI:"ht", UGA:"ug", ZAM:"zm", CHN:"cn", BHR:"bh", BEN:"bj", THA:"th", PLE:"ps", BLR:"by", GUA:"gt", LUX:"lu", VIE:"vn", SLV:"sv" };
function standingsFlagUrl(code) { const iso = FLAG_ISO_CODES[String(code).toUpperCase()]; return iso ? `https://flagcdn.com/w80/${iso}.png` : flagUrl(code); }
function flagEmoji(code) { const iso = FLAG_ISO_CODES[String(code).toUpperCase()] || "un"; if (iso.startsWith("gb-")) return "🇬🇧"; return [...iso.slice(0, 2).toUpperCase()].map((letter) => String.fromCodePoint(127397 + letter.charCodeAt(0))).join(""); }
...
function createObstacles() {
  ...
  const featurePositions = Array.from({ length: 8 }, (_, index) => 0.12 + index * (0.76 / 7));
  featurePositions.forEach((ratio, index) => {
    const x = TRACK.left + TRACK_LENGTH * ratio, baseY = random(170, 530);
    const body = Matter.Bodies.rectangle(x, baseY, 180, 18, { label: "moving-wall", isStatic: true, restitution: 0.25 });
    obstacles.push({ type: "moving-wall", body, x, baseY, speed: random(0.7, 1.15), phase: index * 1.9 });
  });
  featurePositions.forEach((ratio, index) => {
    const x = TRACK.left + TRACK_LENGTH * ratio + random(-160, 160), y = random(150, 550), radius = 26;
    const body = Matter.Bodies.circle(x, y, radius, { label: "bumper", isStatic: true, restitution: 1.45, friction: 0 });
    obstacles.push({ type: "bumper", body, x, y, radius });
  });
  featurePositions.forEach((ratio) => {
    const x = TRACK.left + TRACK_LENGTH * ratio + random(-110, 110), y = (TRACK.top + TRACK.bottom) / 2;
    const body = Matter.Bodies.rectangle(x, y, 28, TRACK.bottom - TRACK.top - 26, { label: "boost-pad", isStatic: true, isSensor: true });
    obstacles.push({ type: "boost", body, x, y, width: 28, height: TRACK.bottom - TRACK.top - 26 });
  });
  featurePositions.forEach((ratio, index) => {
    const x = TRACK.left + TRACK_LENGTH * ratio + random(-120, 120), baseY = TRACK.top - 130;
    const body = Matter.Bodies.rectangle(x, baseY, 20, 240, { label: "shutter", isStatic: true, restitution: 0.35 });
    obstacles.push({ type: "shutter", body, x, baseY, phase: index * 0.73 });
  });
  for (let index = 0; index < 16; index += 1) {
    const x = TRACK.left + TRACK_LENGTH * (0.06 + index * (0.88 / 15));
    const y = index % 2 === 0 ? TRACK.top + 78 : TRACK.bottom - 78;
    const radius = 22;
    const body = Matter.Bodies.circle(x, y, radius, { label: "lane-bumper", isStatic: true, restitution: 1.2, friction: 0 });
    obstacles.push({ type: "lane-bumper", body, x, y, radius });
  }
  return obstacles;
}
function rebuildPhysics(runners, obstacles) {
  ...
  physicsBodies.moving = obstacles.filter((o) => ["moving", "moving-wall", "shutter"].includes(o.type));
  ...
  Matter.Events.on(physics, "collisionStart", ({ pairs }) => {
    for (const pair of pairs) {
      ...
      if (runnerBody && otherBody.label === "boost-pad") {
        const runner = runners.find((item) => item.body === runnerBody);
        if (runner) { Matter.Body.setVelocity(runner.body, { x: Math.min(15, runner.body.velocity.x + 5), y: runner.body.velocity.y * 0.55 }); runner.boost = 0.6; }
      } else if (runnerBody && ["spinner", "moving-obstacle", "moving-wall", "bumper", "lane-bumper", "shutter"].includes(otherBody.label)) {
        const runner = runners.find((item) => item.body === runnerBody);
...
function drawObstacle(o) { ctx.save(); const { x, y } = o.body.position; if (["moving", "bumper", "lane-bumper"].includes(o.type)) { ctx.fillStyle = o.type === "bumper" ? "#ed7045" : o.type === "lane-bumper" ? "#6b62d9" : "#28384d"; ctx.beginPath(); ctx.arc(x, y, o.radius, 0, Math.PI * 2); ctx.fill(); ctx.strokeStyle = o.type === "bumper" ? "#8d301c" : o.type === "lane-bumper" ? "#39328b" : "#efb53e"; ctx.lineWidth = 3; ctx.stroke(); } else if (o.type === "boost") { ctx.fillStyle = "rgba(31, 184, 119, .3)"; ctx.fillRect(x - o.width / 2, y - o.height / 2, o.width, o.height); ctx.strokeStyle = "#15804f"; ctx.lineWidth = 3; ctx.strokeRect(x - o.width / 2, y - o.height / 2, o.width, o.height); ctx.fillStyle = "#12643f"; ctx.font = "bold 24px system-ui"; ctx.fillText("➜", x - 12, y + 8); } else if (o.type === "shutter" && y < TRACK.top - 100) { ctx.restore(); return; } else { ctx.translate(x, y); ctx.rotate(o.body.angle); ctx.fillStyle = o.type === "spinner" ? "#cb3d4e" : o.type === "shutter" ? "#d99d23" : "#56677c"; const width = o.length || (o.type === "moving-wall" ? 180 : 20), height = o.type === "shutter" ? 240 : 14; ctx.fillRect(-width / 2, -height / 2, width, height); if (o.type === "spinner") { ctx.fillStyle = "#7e2234"; ctx.beginPath(); ctx.arc(0, 0, 7, 0, Math.PI * 2); ctx.fill(); } } ctx.restore(); }
...
function tick(now) { if (!race) return; const dt = Math.min(0.05, Math.max(0.001, (now - lastFrame) / 1000)); lastFrame = now; race.elapsed += dt; race.accumulator += dt * 1000; const fixedStep = 1000 / 120; while (race.accumulator >= fixedStep) { for (const runner of race.runners) { if (runner.finished) continue; runner.recovery = Math.max(0, runner.recovery - fixedStep / 1000); const recovering = runner.recovery > 0; Matter.Body.applyForce(runner.body, runner.body.position, { x: recovering ? 0.0018 : 0.00042, y: 0 }); const maxSpeed = recovering ? 12 : 8; const velocity = runner.body.velocity; if (velocity.x > maxSpeed) Matter.Body.setVelocity(runner.body, { x: maxSpeed, y: velocity.y }); if (velocity.x < 2) Matter.Body.setVelocity(runner.body, { x: 2, y: velocity.y }); } for (const obstacle of physicsBodies.moving) { let y; if (obstacle.type === "moving") y = obstacle.baseY + Math.sin(race.elapsed * obstacle.speed + obstacle.phase) * 190; else if (obstacle.type === "moving-wall") y = Math.max(TRACK.top + 100, Math.min(TRACK.bottom - 100, obstacle.baseY + Math.sin(race.elapsed * obstacle.speed + obstacle.phase) * 250)); else { const cycle = (race.elapsed + obstacle.phase) % 5.5; y = cycle < 3.2 ? (TRACK.top + TRACK.bottom) / 2 : TRACK.top - 130; } Matter.Body.setPosition(obstacle.body, { x: obstacle.x, y }); } Matter.Engine.update(physics, fixedStep); race.accumulator -= fixedStep; } for (const runner of race.runners) if (!runner.finished && runner.body.position.x >= TRACK.left + TRACK_LENGTH - runner.radius) { runner.finished = true; race.finishers.push(runner); } drawFrame(); if (race.finishers.length >= race.target) return completeRound(); animationId = requestAnimationFrame(tick); }
function renderStandings(list) { standingsList.innerHTML = list.slice(0, 16).map((team, index) => `<li><strong>${index + 1}</strong><span class="flag-fallback" aria-hidden="true" hidden>${flagEmoji(team.code)}</span><img class="mini-flag" src="${standingsFlagUrl(team.code)}" alt="${escapeHtml(team.name)} 국기" loading="lazy" onerror="this.hidden=true;this.previousElementSibling.hidden=false"><span>${escapeHtml(team.name)}</span><span class="rank-note">FIFA #${team.rank}</span></li>`).join(""); }
